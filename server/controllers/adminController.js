  ...
  try {
    ...
    if (req.file) {
      ...
      try {
        ...
        fs.unlinkSync(filePath); // to remove the file from uploads folder after uploaded to the cloudinary
        fileUrl = result.secure_url; // res.secure_url is the image url
      } catch (error) {
        console.log(error);
      }
...
module.exports.updateConfessionGroup = async (req, res) => {
  try {
    ...
    let confessionGroup = await ConfessionGroup.findByIdAndUpdate(
      req.params.id,
      updateData,
      {
        new: true,
...
module.exports.deleteConfessionGroup = async (req, res) => {
  try {
    const confessionGroup = await ConfessionGroup.findByIdAndDelete(
      req.params.id
    );
...
module.exports.getGroupConfessions = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Confessions fetched successfully", confessions });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Confessions fetched failed", error: error.message });
  }
};
...
module.exports.deleteConfession = async (req, res) => {
  try {
    ...
    const confessionExisted = updatedGroup.confessions.some(
